...
describe("User Service", () => {
  ...
  it("should throw error when user ID is missing", async () => {
    ...
    // ACT & ASSERT
    await expect(fetchUserProfile(null, mockApiClient)).rejects.toThrow(
      "User ID is required",
    );
  ...
  it("should handle 404 error correctly", async () => {
    ...
    // ACT & ASSERT
    await expect(fetchUserProfile(999, mockApiClient)).rejects.toThrow(
      "User not found",
    );
  });
  ...
  it("should handle network errors", async () => {
    ...
    // ACT & ASSERT
    await expect(fetchUserProfile(1, mockApiClient)).rejects.toThrow(
